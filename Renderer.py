...
class Render(object):
  ...
  def point(self, x, y, color = None):
    ...
    try:
      self.pixels[y][x] = color or self.current_color.getColor()
    except:
  ...
  def triangle(self):
    ...
    for x in range(bbox_min.x, bbox_max.x + 1):
      ...
      for y in range(bbox_min.y, bbox_max.y + 1):
        ...
        if self.active_texture and self.withTexture:
          tx = tA.x * w + tB.x * v + tC.x * u
          ty = tA.y * w + tB.y * v + tC.y * u

          color = self.active_texture.get_color(tx, ty, intensity)
        elif self.withColor:
            color = self.current_color.getColor(intensity)
        else:
            color = WHITE
  ...
  def shade_canvas(self, shader):
      for y, aY in enumerate(self.pixels):
          for x, aX in enumerate(self.pixels):
              self.pixels[y][x] = shader(self.width, self.height, x,y)

  def shade_canvas_conditioned(self, shader, condition):
      for y, aY in enumerate(self.pixels):
          for x, aX in enumerate(self.pixels):
              if condition(self.width, self.height, x,y):
                self.pixels[y][x] = shader(self.width, self.height, x,y)
          ...
          for facepart in face:
              try:
                ...
                vertex_buffer_object.append(tvertex)
                  ...
                  tvertex = V3(*model.tvertices[facepart[0]])
                  vertex_buffer_object.append(tvertex)
